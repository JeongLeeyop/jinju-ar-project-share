  ...
  <div v-loading="loading">
    <div class="orderlist-wr">
      <div class="orderlist-info">
        ...
        <div class="shopname">
          <p class="orderlist-ttl"> 거점명 : </p>
          <p class="orderlist-ttl" v-if="orderDetail[0].group.station"> {{ orderDetail[0].group.station.name }}</p>
          <p class="orderlist-ttl" v-else> 거점 정보를 찾을 수 없습니다.</p>
        </div>
        <div class="memo">
          <p class="orderlist-ttl"> 현관 비밀번호 : </p>
          <p class="orderlist-ttl"> {{ orderDetail[0].group.memo }}</p>
        </div>
        <div class="date">
      ...
      <div class="orderlist">
        <div class="list" v-for="item in orderDetail" :key="item.idx">
          <p class="product"><slot v-if="item.product.extraProductStatus">[추가상품] </slot>{{ item.productName }}</p>
          <p class="num">x {{ item.productNum }}개</p>
    ...
    <el-form>
      <el-form-item label="주문상태">
        <el-select v-model="pickupStatus" placeholder="주문상태를 변경해주세요">
          ...
          -->
          <el-option label="배송대기" :value="0"></el-option>
          <el-option label="배송중" :value="1"></el-option>
          <el-option label="배송완료" :value="2"></el-option>
        </el-select>
      </el-form-item>
    </el-form>
