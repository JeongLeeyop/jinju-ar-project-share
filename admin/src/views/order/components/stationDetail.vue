      ...
      <div class="item">
        <div class="orderlist">
          <div v-for="(item, index) in datas" :key="index" class="list">
            <p class="product"><slot v-if="item.extraProductStatus === 1">[추가상품] </slot>{{ item.productName }} x {{ item.totalProductNum | parseKrw }}개</p>
...
import { getProductOrderDetailByStation, updateProductReceiveStatus } from '@/api/productOrder';
...
export default class extends Vue {
  @Prop({ required: true }) private selectedStation!: any;
  ...
  private getProductOrderDetailByProduct() {
    this.loading = true;
    getProductOrderDetailByStation(this.selectedStation.stationId, this.listQuery).then((res) => {
      this.datas = res.data;
      this.loading = false;
    });
  }
  ...
  private handleUpdateStatus() {
    ...
    this.$confirm('정말 입고상태를 수정하시겠습니까?', '입고상태 수정', {
      ...
      updateProductReceiveStatus(this.listQuery, { ...this.form, idxList: [this.selectedStation.stationId] }).then(() => {
