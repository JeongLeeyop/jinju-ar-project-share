					...
					<div class="user__tab">
						<div class="user__subtab">
							<div class="type-wr">
								...
								<el-button @click="$router.push({ name: 'Item' })" class="button-bg01 active">상품별</el-button>
      ...
      <el-table-column label="주문번호" width="">
        <template slot-scope="scope">
          {{ getNumber(scope.$index) }}
				...
				<el-table-column prop="productName" label="상품명" width="">
          <template slot-scope="scope">
            <span v-if="scope.row.extraProductStatus">[추가상품] </span>{{ scope.row.productName }}
          </template>
        </el-table-column>
...
import { getProductOrderListByProduct, getProductOrderTotalStatistics, getExcel } from '@/api/productOrder';
