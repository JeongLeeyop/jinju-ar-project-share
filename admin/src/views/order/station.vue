<template>
	<div class="user-wrap">
				...
				<div class="user-title">
					<div class="user__tab">
						<div class="user__subtab">
							<div class="type-wr">
								<el-button @click="$router.push({ name: 'OrderList' })" class="button-bg01">요일별</el-button>
								<el-button @click="$router.push({ name: 'Item' })" class="button-bg01">상품별</el-button>
								<el-button @click="$router.push({ name: 'Station' })" class="button-bg01 active">거점별</el-button>
							</div>
							<div class="search-wr">
								<el-select v-model="listQuery.searchType" slot="prepend" placeholder="검색분류">
									<el-option label="이름" value="actualName" />
									<el-option label="픽업매장" value="stationName" />
      ...
      >
				...
				<el-table-column label="거점번호" width="">
          <template slot-scope="scope">
            {{ getNumber(scope.$index) }}
          </template>
        </el-table-column>
				<el-table-column label="거점명" width="">
          <template slot-scope="scope">
            <slot v-if="scope.row.stationName">{{ scope.row.stationName }}</slot>
            <slot v-else>삭제된 거점</slot>
          </template>
        </el-table-column>
				<el-table-column label="거점위치" width="">
          <template slot-scope="scope">
            <slot v-if="scope.row.stationName">{{ scope.row.address }} {{ scope.row.addressDetail }}</slot>
            <slot v-else></slot>
          </template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            <el-button v-if="scope.row.stationName" class="detail-button" type="text" @click="handleViewStation(scope.row)">요약보기</el-button>
          </template>
				</el-table-column>
			</el-table>
			<Pagination
      ...
      />
		...
		<el-dialog class="order-dialog-wr" :title="`요약보기 : ${selectedStation ? selectedStation.stationName : ''}`" :visible.sync="viewVisible">
      <StationDetail
        v-if="viewVisible"
        :selectedStation="selectedStation"
        :listQuery="listQuery"
        @close="handleViewStation(null)"
      />
		</el-dialog>
...
export default class extends Vue {
  ...
  private selectedStationIdxList: any = [];
  ...
  /* eslint-disable */
    ...
    this.selectedStationIdxList = selection.map((item: any) => item.stationId);
  ...
  private handleViewStation(order: any) {
    this.selectedStation = order;
  ...
  private handleUpdateReceiveStatus() {
    ...
    this.$confirm('정말 입고상태를 일괄 수정하시겠습니까?', '입고상태 일괄 수정', {
      ...
      updateProductReceiveStatus(this.listQuery, { idxList: this.selectedStationIdxList, receiveStatus: this.receiveStatus }).then(() => {
        this.$message.success('입고상태가 수정되었습니다.');
        this.selectedStationIdxList = [];
