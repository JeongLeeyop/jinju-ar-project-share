				...
				<div class="user-title">
					<div class="user__tab">
						<div class="user__subtab">
							<div class="type-wr">
								<el-button @click="$router.push({ name: 'OrderList' })" class="button-bg01 active">요일별</el-button>
								<el-button @click="$router.push({ name: 'Item' })" class="button-bg01">상품별</el-button>
								<el-button @click="$router.push({ name: 'Station' })" class="button-bg01">거점별</el-button>
							</div>
							<div class="search-wr">
								<el-select v-model="listQuery.searchType" slot="prepend" placeholder="검색분류">
									<el-option label="이름" value="actualName" />
									<el-option label="거점명" value="stationName" />
      ...
      >
          ...
          <template slot-scope="scope">
            {{ scope.row.name }}
				...
				<el-table-column label="배송지" width="200">
          <template slot-scope="scope">
            <slot v-if="scope.row.stationName">
            ...
            <slot v-else>
              삭제된 거점
				...
				<el-table-column label="주소" width="300">
          <template slot-scope="scope">
          ...
          <template slot-scope="scope">
            {{ scope.row.productNum | parseKrw }}개
				...
				<el-table-column label="배송날짜" width="">
          <template slot-scope="scope">
          ...
          <template slot-scope="scope">
            <el-tag v-if="scope.row.pickupStatus == 0">배송대기</el-tag>
            <el-tag v-else-if="scope.row.pickupStatus == 1">배송중</el-tag>
            <el-tag v-else-if="scope.row.pickupStatus == 2">배송완료</el-tag>
            <el-tag v-else>배달 대기</el-tag>
          </template>
        </el-table-column>
...
import { getExcel, getProductOrderTotalStatistics, getProductOrderListByDay } from '@/api/productOrder';
...
export default class extends Vue {
  ...
  private handleExcel() {
    this.loading = true;
    getExcel({ ...this.listQuery, excelType: 'day' }).then((res) => {
