    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <div class="header-info">
          <h1 class="page-title">공간 가입자 목록</h1>
          <p class="page-subtitle">{{ spaceName }} - 총 {{ totalElements }}명</p>
        </div>
      </div>
...
import { getSpaceMembers, getSpaceDetail } from '@/api/space';
...
export default class extends Vue {
  ...
  private async loadSpaceInfo() {
    const uid = this.$route.params.uid;
    try {
      const res = await getSpaceDetail(uid);
  ...
  private async loadMembers() {
    ...
    this.loading = true;
    try {
      const res = await getSpaceMembers(uid, this.listQuery);
