	...
	<div class="user-wrap">
		<div class="user-head order detail">
			<div class="user-title-wr">
				<div class="user-title">
					<div class="tl__box">
						<p class="tl txt1">매장별 정렬 :</p>
						<p class="tl txt2">윙스타워점</p>
						<p class="usernumber">총 789건</p>
					</div>
					<div class="user__tab">
						<div class="user__subtab">
							<div class="type-wr">
								<div class="setting__btn__box"><button @click="$router.push({ name: 'Shop2' })"
										class="cancel">뒤로가기</button></div>
							</div>
							<div class="search-wr">
								<el-select v-model="select" slot="prepend" placeholder="검색분류">
									<el-option label="주문번호" value="id" />
									<el-option label="이름" value="userName" />
									<el-option label="주문품목" value="orderItem" />
								</el-select>
				...
				<div class="user-title2">
					...
					<div class="user__tab">
						<div class="user__subtab">
							<div class="search-wr">
								<el-button class="button-bg01" :class="{ active: isActive1 }"
									@click="setToggle('1')">월</el-button>
								<el-button class="button-bg01" :class="{ active: isActive2 }"
									@click="setToggle('2')">화</el-button>
								<el-button class="button-bg01" :class="{ active: isActive3 }"
									@click="setToggle('3')">수</el-button>
								<el-button class="button-bg01" :class="{ active: isActive4 }"
									@click="setToggle('4')">목</el-button>
								<el-button class="button-bg01" :class="{ active: isActive5 }"
									@click="setToggle('5')">금</el-button>
								<el-button class="button-bg01" :class="{ active: isActiveAll }"
									@click="setToggleAll()">전체</el-button>
							</div>
							<div class="search-wr">
								<p class="subtab-title">검색기간</p>
								<el-date-picker v-model="dateRange" readonly backgroundColor="grey" type="daterange"
									align="right" range-separator="~" value-format="yyyy-MM-dd" />
							</div>
						</div>
						<!-- <router-link :to="{ name: 'OrderAdd' }">추가 +</router-link> -->
					</div>
		...
		<div class="user-content order">
			<el-table :data="tableData" :header-cell-style="{ background: '#9097a4', color: '#fff', padding: '5px 0' }"
				...
				<el-table-column prop="qty" label="상품 수량" width=""></el-table-column>
