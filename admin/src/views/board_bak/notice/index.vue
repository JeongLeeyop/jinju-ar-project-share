<template>
	<div class="user-wrap">
			...
			<div class="user__tab">
				<div class="user__subtab">
					...
					<el-select v-model="select" slot="prepend" placeholder="">
						<el-option label="제목" value="noticeTitle"></el-option>
					</el-select>
					<el-input placeholder="" v-model="input" class="search"></el-input>
					<el-button><img src="~@/assets/images/search.png" alt=""></el-button>
				</div>

				<router-link :to="{ name: 'BoardNoticeAdd' }">추가 +</router-link>
			</div>
		</div>

		<div class="user-content board">
			<el-table :data="notices" :header-cell-style="{ background: '#9097a4', color: '#fff', padding: '5px 0' }"
				:default-sort="{ prop: 'date', order: 'descending' }" style="width: 100%;" border>
				<el-table-column prop="id" label="번호" width="70"></el-table-column>
				<el-table-column prop="noticeTitle" label="제목" class-name="left"></el-table-column>
				<el-table-column prop="noticeFile" label="첨부파일" width="130"></el-table-column>
				<el-table-column prop="noticeDay" label="작성일" sortable width="200" class-name="sort"></el-table-column>
				<el-table-column prop="noticeWatch" label="조회수" sortable width="100" class-name="sort"></el-table-column>
		...
		<el-dialog title="공지사항" class="board-detail" :visible.sync="dialogTableVisible">
			<div class="board-wr">
				<div class="board-header">
				<div class="board-ttl">웨일리잇 공지사항입니다.</div>
				...
				<div class="board-content">
					...
					<ul>
						<li class="board-button-wr">
							<router-link class="board-button btn01" :to="{ name: 'BoardNoticeEdit' }">
								수정하기
							</router-link>
						</li>
					</ul>
...
export default {
	...
	created() {
		for (let i = 0; i < 1000; i += 1) {
			this.tableData.push({
				id: `${i}`,
				noticeTitle: `${notice[i].noticeTitle}`,
				noticeField: `${notice[i].noticeField}`,
				noticeFile: `${notice[i].noticeFile}`,
				noticeDay: `${notice[i].noticeDay}`,
				noticeWatch: `${notice[i].noticeWatch}`,
			});
		}
	},
	data() {
