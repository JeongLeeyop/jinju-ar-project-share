  ...
  private async getVideo() {
    await getVideo(this.$route.params.videoIdx).then((res) => {
        ...
        console.log('영상 데이터 로드 완료:', {
          ...
          myWatchPercent: this.videoData.myWatchPercent,
          lastWatchSecond: this.videoData.lastWatchSecond,
          lastWatchUpdate: this.videoData.lastWatchUpdate,
        });
        this.loading = false;
      }).catch(() => {
  ...
  private saveWatchHistory() {
    ...
    saveWatchHistory(this.watchHistoryData).then((res: any) => {
      console.log('현재 시청 구간 저장완료', {
        ...
        lastWatchSecond: this.watchHistoryData.lastWatchSecond,
      });
    }).catch((error: any) => {
      console.error('시청 기록 저장 실패:', error);
      if (error.response?.status === 401) {
        console.log('인증되지 않은 사용자입니다. 시청 기록을 저장하지 않습니다.');
      }
    });
  }
