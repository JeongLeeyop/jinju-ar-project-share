<template>
    <div>
        <el-input v-if="typeCode === 'TEXT'" v-model="inputValue" type="text" :maxlength="inputLimit" />
        <el-input v-if="typeCode === 'NUMBER'" v-model="inputValue" type="number" :maxlength="inputLimit" />
        <el-input v-if="typeCode === 'TEXT_AREA'" v-model="inputValue" type="textarea" :maxlength="inputLimit" />
        <el-input v-if="typeCode === 'EMAIL'" v-model="inputValue" type="email" :maxlength="inputLimit" />
        <el-date-picker v-if="typeCode === 'DATE'" v-model="inputValue" type="date" value-format="yyyy-MM-dd" />
        <el-input v-if="typeCode === 'TEL'" v-model="inputValue" type="text" placeholder="000-000-0000" />
        <slot v-if="typeCode === 'ADDRESS'">
            <el-button @click="handleAddressClick">
                주소 검색
            </el-button>
            <el-input disabled v-model="inputValue" :maxlength="inputLimit" />
            <el-dialog title="주소 검색" center class="dialog--no-padding dialog-wrap" :visible.sync="addressDialogVisible"
                :before-close="handleAddressClick" append-to-body>
                <span>
                    <!--
        ...
        <editor v-if="typeCode === 'EDITOR'" :inputValue="inputValue" :moduleName="moduleName"
            @changeValue="handleChange" />
        <el-upload v-if="typeCode === 'FILE'" :action="`${apiUrl}${moduleName}/upload`" :headers="requestHeaders"
            :before-upload="handleBeforeUpload" :on-success="handleSuccessUpload" :limit="inputLimit"
            :file-list="getFileList()" :on-remove="handleRemove">
        ...
        <el-upload v-if="typeCode === 'PHOTO'" list-type="picture" :on-success="handleSuccessUpload"
            :action="`${apiUrl}${moduleName}/upload`" :headers="requestHeaders"
            :before-upload="(file) => handleBeforeUpload(file, true)" :limit="inputLimit" :file-list="getFileList()"
