...
export default class extends Vue {
  ...
  private async checkAllPermissions() {
    ...
    try {
      ...
      // 채널 생성자 체크
      this.isChannelCreator = channel?.userUid === currentUserInfo.uid;

      // 권한 체크 병렬 실행
      try {
        const [spaceCreateResponse, marketplaceOfflineResponse] = await Promise.all([
          checkPermissionByUser(this.currentChannelUid, 'SPACE_CREATE'),
          checkPermissionByUser(this.currentChannelUid, 'MARKETPLACE_OFFLINE_REGISTER'),
        ]);
