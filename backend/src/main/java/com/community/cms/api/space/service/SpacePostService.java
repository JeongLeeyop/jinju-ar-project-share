...
public class SpacePostService {
    ...
    public SpacePostDto getPost(String postUid, String currentUserUid) {
        ...
        // 작성자 아이콘 조회
        String iconFileUid = getUserIconFileUid(post.getUserUid());

        return SpacePostDto.from(post, currentUserUid, isLiked, iconFileUid);
    }
    ...
    public List<SpacePostDto> getPostsBySpace(String spaceUid, String currentUserUid) {
        ...
        for (int i = 0; i < fixedNoticeCount; i++) {
            ...
            if (!post.isHidden() || 
                ...
                String iconFileUid = getUserIconFileUid(post.getUserUid());
                result.add(SpacePostDto.from(post, currentUserUid, isLiked, iconFileUid));
            }
        }
    ...
    /**
     * User의 iconFileUid 조회 헬퍼 메서드
     */
    private String getUserIconFileUid(String userUid) {
        if (userUid == null) {
            return null;
        }
        return userRepository.findByUid(userUid)
                .map(User::getIconFileUid)
                .orElse(null);
    }
    ...
    public List<SpacePostCommentDto> getCommentsByPost(String postUid, String currentUserUid) {
        ...
        return comments.stream()
                .map(comment -> {
                    String iconFileUid = getUserIconFileUid(comment.getUserUid());
                    return SpacePostCommentDto.from(comment, currentUserUid, iconFileUid);
                })
                .collect(Collectors.toList());
    }
