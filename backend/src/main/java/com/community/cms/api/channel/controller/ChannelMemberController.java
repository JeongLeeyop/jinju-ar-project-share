...
public class ChannelMemberController {
    ...
    public ResponseEntity remove(
            @AuthenticationPrincipal SinghaUser authUser,
            @PathVariable("uid") ChannelMember channelMember) {
        System.out.println("=== 추방 API 호출 ===");
        System.out.println("PathVariable uid: " + channelMember.getIdx());
        System.out.println("회원 정보 - IDX: " + channelMember.getIdx() + ", UserUID: " + channelMember.getUserUid());
        
        channelMemberService.remove(channelMember, authUser);
        
        System.out.println("=== 추방 API 응답 전송 ===");
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity unban(
            @AuthenticationPrincipal SinghaUser authUser,
            @PathVariable("uid") ChannelMember channelMember) {
        channelMemberService.unban(channelMember, authUser);
        return ResponseEntity.ok().build();
    }
}
