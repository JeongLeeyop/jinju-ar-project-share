...
import com.community.cms.api.lession.service.AdmLessionService;
...
@AllArgsConstructor
public class AdmLessionController {
    private final AdmLessionService admLessionService;
    private final AttachedFileService attachedFileService;

    @GetMapping
    public ResponseEntity<Page<LessionDto.detail>> list(@AuthenticationPrincipal SinghaUser authUser, LessionSearch search, @PageableDefault(direction = Direction.DESC, sort = { "createDate" }) Pageable pageable) {
        return ResponseEntity.ok(admLessionService.list(authUser, search, pageable));
    }
    
    @PostMapping
    public ResponseEntity add(@AuthenticationPrincipal SinghaUser authUser, @RequestBody LessionDto.add addDto) {
        admLessionService.add(authUser, addDto);
    ...
    public ResponseEntity update(@AuthenticationPrincipal SinghaUser authUser, @PathVariable("uid") Lession Lession,@RequestBody LessionDto.update updateDto) {
        admLessionService.update(authUser, Lession, updateDto);
        return ResponseEntity.ok().build();
    }
	...
	public ResponseEntity<AttachedFileDto.detail> fileUpload(MultipartFile file) {
		return ResponseEntity.ok(AttachedFileMapper.INSTANCE.entityToDetailDto(attachedFileService.save(file, "Lession")));
	}
}
