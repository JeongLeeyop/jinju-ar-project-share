package com.community.cms.api.post.repository.query;
...
public class PostQuery {
    ...
    public List<PostDto.ClientList> list(PostSearch search, Pageable pageable, SinghaUser authUser) {
        ...
        sql.append("    (SELECT idx FROM post_like pl WHERE pl.post_uid = p.uid AND pl.user_uid = :userUid), \n"); // 10
        sql.append("    (SELECT COUNT(*) FROM comment c WHERE c.post_uid = p.uid AND hide = false AND delete_status = false), \n"); // 11
        ...
        sql.append("    CASE WHEN p.user_uid = :userUid THEN 1 END \n"); // 17
        ...
        sql.append("LEFT JOIN user u ON p.user_uid = u.uid \n");
        ...
        if (StringUtils.hasText(search.getChannelUid())) sql.append("AND p.channel_uid = :channelUid \n");
        ...
        if (search.getCategoryList() != null && search.getCategoryList().size() > 0) {
            sql.append("AND pc.category_uid IN :categoryList \n");
        }

        sql.append("GROUP BY p.uid, pc.category_uid \n");

        if (StringUtils.hasText(search.getSort()) && search.getSort().equals("notice")) {
            sql.append("ORDER BY p.notice_status DESC, p.create_date DESC \n");
        } else if (StringUtils.hasText(search.getSort()) && search.getSort().equals("hot")) {
        ...
        if (StringUtils.hasText(search.getChannelUid())) {
            query.setParameter("channelUid", search.getChannelUid());
        }
        ...
        for (Object[] row : resultList) {
            ...
            if (row[++index] != null) data.setNoticeStatus(row[index].toString().equals("true"));
            if (row[++index] != null) data.setIconFileUid(row[index].toString());
            ...
            if (row[++index] != null) data.setLikeIdx(Integer.parseInt(row[index].toString()));
            ...
            if (row[++index] != null) data.setMyStatus(row[index].toString().equals("1"));
