...
public interface AdmChannelLevelService {
...
@Service
@AllArgsConstructor
class AdmChannelLevelServiceImpl implements AdmChannelLevelService {
...
private final UserRepository userRepository;
    ...
    channelUid = addDtoList.get(0).getChannelUid();
            // TODO 한번에 입력하는 방법
            for(ChannelLevelDto.add dto : addDtoList) {
                Optional<ChannelLevel> optional = channelLevelRepository.findByChannelUidAndLevel(channelUid, dto.getLevel());
                ChannelLevel entity;
                if(optional.isPresent()){
                    entity = optional.get();
                    ChannelLevelMapper.INSTANCE.updateDtoToEntity(dto, entity);
